<div class="min-h-screen bg-slate-50 flex items-center justify-center p-4 sm:p-8 font-sans"
    x-data="{
        ...
        <div class="w-full md:w-[350px] lg:w-[400px] bg-slate-900 relative flex flex-col justify-between p-10 text-white overflow-hidden flex-shrink-0">
            ...
            <div class="relative z-10">
                ...
                <p class="text-slate-400 text-sm">
                    {{ __('installer::installer.step_of', ['current' => array_search($step->id(), array_keys($steps)) + 1, 'total' => count($steps)]) }}
                </p>
            </div>
            ...
            <div class="relative z-10 mt-12 flex-1">
                <div class="space-y-6">
                    ...
                    @foreach($steps as $s)
                        ...
                        @endphp
                        <{{ $isPast ? 'button type=button wire:click=goToStep("'.$s->id().'")' : 'div' }} class="w-full text-left flex items-center group transition-all duration-300 {{ $isCurrent ? 'opacity-100 translate-x-1' : ($isPast ? 'opacity-50 hover:opacity-100 cursor-pointer' : 'opacity-30') }}">
                            <div class="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border transition-all duration-300
                                {{ $isCurrent ? 'bg-white text-slate-900 border-white shadow-lg scale-110' : ($isPast ? 'bg-blue-600/20 border-blue-500/50 text-blue-200 group-hover:bg-blue-500/40' : 'border-slate-700 text-slate-500') }}">
        ...
        <div class="w-full md:w-2/3 bg-white p-8 md:p-12 flex flex-col relative">
            ...
            <form wire:submit.prevent="next" class="flex-1 flex flex-col justify-between h-full">
                ...
                <div class="mt-12 flex items-center justify-between border-t border-gray-100 pt-6">
                    @if(!$isFirstStep)
                        <button type="button" wire:key="back-btn" wire:click="previous" wire:loading.attr="disabled"
                            class="inline-flex items-center px-6 py-3 text-sm font-semibold text-slate-600 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 hover:text-slate-800 transition-all duration-200 disabled:opacity-50 cursor-pointer">
